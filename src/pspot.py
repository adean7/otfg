...
class Pseudopotential:
    ...
    def allocate(self, currentParams, model):
        lmax = 4
        ...
        self.V_loc = np.zeros(model.num_points)

        # Local part of ionic psp, in real space x r.
        self.V_locr = np.zeros(model.num_mesh)

        # Derivative of local part of ionic psp wrt g.
        self.dV_loc_dg = np.zeros(model.num_points)

        self.has_V_tau = False

        # Local part of ionic MGGA in reciprocal space.
        self.V_tau = np.zeros(model.num_points)

        # Local part of ionic MGGA in real space.
        self.V_taur = np.zeros(model.num_mesh)

        # Derivative of local part of ionic MGGA wrt g.
        self.dV_tau_dg = np.zeros(model.num_points)

        # Number of projectors.
        self.num_ps_projectors = 0

        # q_nm for each pair of projectors.
        self.q = np.zeros((model.num_projectors, model.num_projectors))

        # D^0_nm for each pair of projectors.
        self.D0 = np.zeros((model.num_projectors, model.num_projectors))

        # Assume the pseudopotential does not include spin orbit coupling unless otherwise specified.
        self.SOC = currentParams.SOC

        # Angular momentum, l, for each projector.
        self.projector_l = np.zeros(model.num_projectors, dtype=int)

        # Angular momentum, m, for each projector.
        self.projector_m = np.zeros(model.num_projectors, dtype=int)

        # Twice the total angular momentum, j, for each projector.
        self.projector_2j = np.zeros(model.num_projectors, dtype=int)

        # Is there a core charge for calculating NLCC?.
        self.core_charge = True

        # The radial core charge density.
        self.rs_core_charge = np.zeros(model.num_mesh)
        self.core_radial_charge = np.zeros(model.num_points)

        # Derivative of the radial core charge density.
        self.dcore_radial_charge_dg = np.zeros(model.num_points)

        # The radial core kinetic_energy_density.
        self.rs_core_kinetic_energy_density = np.zeros(model.num_mesh)
        self.core_radial_kinetic_energy_density = np.zeros(model.num_points)

        # Derivative of the radial core kinetic energy density.
        self.dcore_radial_kinetic_energy_density_dg = np.zeros(model.num_points)

        # Non-local part of ionic psp in recip space.
        self.beta = np.zeros((model.num_points, model.num_projectors))

        # Non-local part of ionic psp on a real log grid.
        self.betar = np.zeros((model.num_mesh, model.num_projectors))

        # Non-local part of ionic psp in real space.
        self.beta_real = np.zeros((model.num_points, model.num_projectors))
        ...
        self.rlog = np.zeros(model.num_mesh)
        self.rab = np.zeros(model.num_mesh)

        # The inner core radii for the aug fns.
        self.rinner = np.zeros(2 * lmax + 1)
        self.ninner = np.zeros(2 * lmax + 1)

        # The aug fns on a radial real space grid.
        self.qfunc = np.zeros((model.num_kkbeta, model.num_projectors, model.num_projectors))

        # The kinetic energy density aug fns on a radial real space grid.
        self.tfunc = np.zeros((model.num_kkbeta, 2 * lmax + 1, model.num_projectors, model.num_projectors))

        # The polynomial coefficients of the aug fns.
        self.qfcoef = np.zeros((model.num_nqf, 2 * lmax + 1, model.num_projectors, model.num_projectors))

        # The polynomial coefficients of the aug fns of the kinetic energy density.
        self.tfcoef = np.zeros((model.num_nqf, 2 * lmax + 1, model.num_projectors, model.num_projectors))

        # The l-dependent augmentation functions on a radial grid.
        self.qfunc_with_l = np.zeros(
            (model.num_kkbeta, 2 * lmax + 1, model.num_projectors, model.num_projectors))

        self.qfunc_is_polynomial = True  # pspot defaults to augmentation functions being stored as polynomial expansions
        ...
        # Non-local part of ionic psp in recip space
        self.gamma = np.zeros((model.num_points, model.num_bl_projectors))

        # Non-local part of ionic psp in real space.
        # Angular momentum, l, for each blochl projector.
        self.bl_projector_l = np.zeros(model.num_bl_projectors)

        # Angular momentum, m, for each blochl projector.
        self.bl_projector_m = np.zeros(model.num_bl_projectors)

        # Number of gamma projectors.
        self.num_bl_projectors = 0

        # Number of points in reciprocal radial grid.
        self.bl_num_points = 0
        ...
        self.core_nmr_shift = 0.0
        self.core_mag_sus = 0.0

        # The augmentation factors for each projector pair.
        self.bl_dia_nmrshift = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_para_nmrshift = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_para_damped = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_efg_aug = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_hf_fermi = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_hf_dipolar = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_hf_dipolar_cutoff = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_hf_zora = np.zeros((model.num_bl_projectors, model.num_bl_projectors))

        # Data associated with EPR g-tensor.
        self.bl_zke_aug_gtensor = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_sodia_g_tensor = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        self.bl_sopara_g_tensor = np.zeros((model.num_bl_projectors, model.num_bl_projectors))
        ...
        self.num_core_projectors = 0

        # Number of core radial orbitals.
        self.num_core_radials = 0

        # Principal quantum number of core radials.
        self.core_radials_n = np.zeros(15, dtype=int)

        # Angular momentum l, for each core radials.
        self.core_radials_l = np.zeros(15, dtype=int)

        # The radial core orbitals (reciprocal space).
        self.core_radials = np.zeros((model.num_points, 15, 5))
