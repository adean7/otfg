...
class AllElectronBasis:
    ...
    def init_arrays(self):
        if self.npts == 0:
            io.abort('Trying to initialise basis arrays before npts is defined.')
    ...
    def init_ae_basis(self, Z):
        ...
        self.a = math.exp(-aasf) / float(Z)
        self.b = 1.0 / bbsf

        self.rmax = 400.0  # Set the extent of the logarithmic grid

        self.npts = 2 + int(math.log(self.rmax / self.a + 1.0) / self.b)  # Set the size of the grid
        ...
        for i in range(self.npts):
            self.r[i] = self.a * (math.exp(self.b * float(i)) - 1.0)
            self.rab[i] = self.b * (self.r[i] + self.a)
            self.sqr[i] = math.exp(self.b * float(i + 1) / 2.0)
