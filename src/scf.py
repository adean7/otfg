...
class AllElectronAtom:
    rhorr     : np.array = None
    rhorr_val : np.array = None
    kedrr     : np.array = None
    V_nuc     : np.array = None
    V_eff     : np.array = None
    V_tau     : np.array = None
    V_val     : np.array = None
    ZORA_K    : np.array = None
    ZORA_dKdr : np.array = None
    have_K    : bool     = None
    norbs     : np.array = None
    Z         : int      = None
    nelec     : float    = None
    ...
    def init_atom(self, Z: int, ab: basis.AllElectronBasis, cfg_occ: list, ps: pspot.Pseudopotential=None):
        # Allocate the charge.
        self.rhorr = np.zeros(ab.npts)

        # Allocate the valence charge.
        self.rhorr_val = np.zeros(ab.npts)
        ...
        # Allocate the ZORA K function.
        self.ZORA_K = np.zeros(ab.npts)
        self.ZORA_dKdr = np.zeros(ab.npts)
        ...
        # Determine the number of orbitals in each channel.
        self.nelec = sum(cfg_occ)
            ...
            for i in range(len(data.gs_occ.get(0))):
                ...
                if cfg_occ[i] > 0.0:
                    if l == 0:
                        ...
                        self.norbs[l] = max(self.norbs[l], 2*((data.gs_occ.get(0)[i]-l)/10 - l))
        ...
        # Allocate the hacked real space pseudo orbitals.
        self.orbr_hack = np.zeros((ab.npts, self.norbmax, ab.lmax + 1))
        ...
        self.krel = np.zeros((self.norbmax, ab.lmax + 1), dtype=int)

        self.twoj = np.zeros((self.norbmax, ab.lmax + 1), dtype=int)
        ...
        if ps is not None:
            # If pseudopotential is supplied then use that instead.
            pass
        ...
        for l in range(ab.lmax+1):
            ...
            for no in range(self.norbs[l]):
                ...
                while not solver_converged and niter < max_solve:
                    ...
                    if nctp == 8:
                        io.abort('schroedinger_solve: no classical turning point found')
                    if nctp > ab.npts-11:
                        io.abort('schroedinger_solve: classical turning point too close to ab.npts')
                    ...
                    if ninf > ab.npts:
                        io.abort('schroedinger_solve: ninf too big for this mesh')
                    ...
                    if niter >= max_solve:
                        ...
                        io.abort('schrodinger_solve: could not converge')
    ...
    def set_H(self, ab: basis.AllElectronBasis, mixing: float):
        ...
        for l in range(ab.lmax+1):
            for no in range(self.norbs[l]):
                if self.SOC:
                    self.rhorr += self.occ[no,l] * (self.orbr[:,no,l] ** 2.0 + self.orbr_hack[:,no,l] ** 2.0)
                else:
    ...
    def hartree(self, ab, vhar, nrc=None, Vrc=None):
        ...
        if nrc is not None:
            if Vrc is None:
                io.abort('aeat_hartree: Vrc must be supplied if nrc is not None')
    ...
    def xc(self, ab, vxc, kedrr=None, de_dk=None):
        ...
        if len(self.rhorr) < ab.npts: io.abort('Error, rhorr must have at least ab.npts in ae_xc')
        if len(vxc) < ab.npts: io.abort('Error, vxc must have at least ab.npts in ae_xc')
        ...
        if kedrr is not None:
            if len(kedrr) < ab.npts: io.abort('Error, kedrr must have at least ab.npts in ae_xc')
...
def cycle(currentCell:cell.UnitCell, ab: basis.AllElectronBasis, aeat: AllElectronAtom):
    ...
    print('/===============================================================->')
    print('| Atomic calculation performed for {:>3}: {:<80}'.format(currentCell.speciesSymbol, aeat.config))
    print('|---------------------------------------------------------------->')
